...
try {
    ...
    $finExpoTemp= $_POST['finExposition'] ?? "";    
    ...
    $motCle = htmlentities($motCle, ENT_QUOTES);
...
<div class="container text-center mt-5">
    <h1 class="mb-5 fs-1 text-white fw-bold"><br><br><br>Bienvenue dans la section Gestion des Expositions</h1>

    <!-- Bouton pour exporter les employés en CSV -->
    <form method="post" action="../csv/export_expositions_csv.php"
    ...
    <div class="row row-cols-1 row-cols-md-8 g-4 justify-content-center rounded-3 mb-5 pt-2  navbar-transparent">
        <form method="post" action="page_expositions.php?page=1" class="row justify-content-center">
            ...
            <div class="row " id="details">
                ...
                <div class="col-md-3">
                    ...
                    <select id="debutExposition" name="debutExposition" class="form-select">
                        <option value="" <?php echo ($debutExpoTemp == "") ? 'selected' : ''; ?>>Tous</option>
                        <?php foreach ($tabDebutExposition as $debutExpositionExp): ?>
                            <?php if ($debutExpositionExp == null): ?>
                            <option value="null" <?php echo ($debutExpoTemp === "null") ? 'selected' : ''; ?>>Aucune</option>
                            <?php else: ?>
                ...
                <div class="col-md-3">
                    ...
                    <select id="finExposition" name="finExposition" class="form-select">
                     <option value="" <?php echo ($finExpoTemp == "") ? 'selected' : ''; ?>>Tous</option>
                     <?php foreach ($tabFinExposition as $finExpositionExp): ?>
                           <?php if ($finExpositionExp == null): ?>
                                <option value="null" <?php echo ($finExpoTemp === "null") ? 'selected' : ''; ?>>Aucune</option>
                         <?php else: ?>
            ...
            <!-- Bouton de réinitialisation des filtres -->
                <div class="col-md-3 align-self-end mb-5 text-end offset-md-6">
                    <a href="page_expositions.php?page=1" class="btn btn-danger w-100">Réinitialiser</a>
            </div>
        </div>
    </div>
...
<div id="cardContainer">
    <div class="row row-cols-1 row-cols-md-4 g-4 justify-content-center">
        <?php if (count($tableauExpositions) == 0) { ?>
        ...
        <?php } else { ?>
            <?php foreach ($tableauExpositions as $exposition): ?>
                <div class="cardAffichage shadow-sm border-0 h-100 position-relative me-3">
                    ...
                    <form method="POST" action="page_modif_expositions.php">
                        <?php
                        $idExpo = $exposition['idExposition'];
                    ...
                    <div class="cardAffichage-body text-black rounded text-start">
                        <span class="fs-5 fw-bold">Identifiant :</span> <?php echo($exposition['idExposition']) ?>
                        <br><br><span class="fs-5 fw-bold">Intitulé : </span><?php echo(html_entity_decode($exposition['intitule'], ENT_QUOTES, 'UTF-8')) ?>
                        <br><br><span class="fs-5 fw-bold">Période de Début : </span><?php echo(html_entity_decode($exposition['periodeDebut'], ENT_QUOTES, 'UTF-8')) ?>
                        <br><br><span class="fs-5 fw-bold">Période de Fin : </span><?php echo(html_entity_decode($exposition['periodeFin'], ENT_QUOTES, 'UTF-8')) ?>
                        <br><br><span class="fs-5 fw-bold">Nombre Oeuvres : </span><?php echo(html_entity_decode($exposition['nombreOeuvres'], ENT_QUOTES, 'UTF-8')) ?><br>

                        <!-- Contenu supplémentaire caché par défaut -->
                        <div id="extraContent<?php echo $exposition['idExposition']; ?>" style="display: none;">
                            <!-- Détails supplémentaires sur l'exposition -->
                            <br><span class="fs-5 fw-bold">Resume : </span><?php echo($exposition['resume']) ?>
                            <br><br><span class="fs-5 fw-bold">Debut Exposition : </span><?php echo (html_entity_decode($exposition['debutExpoTemp'], ENT_QUOTES, 'UTF-8') != null ? $exposition['debutExpoTemp'] : "Aucune"); ?>
                            <br><br><span class="fs-5 fw-bold">Fin Exposition : </span><?php echo (html_entity_decode($exposition['finExpoTemp'], ENT_QUOTES, 'UTF-8') != null ? $exposition['finExpoTemp'] : "Aucune"); ?>
                            <br><br><span class="fs-5 fw-bold">Mot Cle de l'Exposition: </span><?php echo(html_entity_decode($exposition['motsCles'], ENT_QUOTES, 'UTF-8')) ?>
                        </div>

                        <button id="toggleContent<?php echo $exposition['idExposition']; ?>"
                                class="btn btn-link p-0"
                                onclick="toggleCardContent('<?php echo $exposition['idExposition']; ?>')">
                            Plus
                        </button>
                    </div>
    ...
    <div class="d-flex justify-content-center align-items-center">
        <table class="table-style">
            ...
            <?php foreach ($tableauExpositions as $expostions): ?>
                <tr>
                    <td><?php echo(html_entity_decode($expostions['idExposition'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['intitule'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['periodeDebut'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['periodeFin'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['nombreOeuvres'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td class="resume"><?php echo(html_entity_decode($expostions['resume'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['debutExpoTemp'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['finExpoTemp'], ENT_QUOTES, 'UTF-8')) ?></td>
                    <td><?php echo(html_entity_decode($expostions['motsCles'], ENT_QUOTES, 'UTF-8')) ?></td>
                    ...
                    <td class="text-center">
                        <a href="#" class="text-danger" data-bs-toggle="modal" data-bs-target="#deleteModalTableau<?php echo $idExpo; ?>">
                            <i class="fas fa-trash-alt fs-4"></i>
                        </a>
                        <!-- Delete Confirmation Modal -->
                        <div class="modal fade" id="deleteModalTableau<?php echo $idExpo; ?>" tabindex="-1" aria-labelledby="deleteModalLabel<?php echo $idExpo; ?>" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="deleteModalLabel<?php echo $idExpo; ?>">Confirmation de suppression</h5>
